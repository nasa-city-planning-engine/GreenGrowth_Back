...
try:
    ...
    print("Failed to initialize Earth Engine. Attempting authentication...")
    ee.Authenticate()
    ee.Initialize(project=os.getenv("GEE_PROJECT"))
    print("✅ Google Earth Engine initialized after authentication.")

# --- Main Geospatial Processing Class ---
class GeoProcessor:
    """
    ...
    def _initialize_vis_params(self):
        ...
        self.temp_vis_params = {'min': -5, 'max': 45, 'palette': ['#000080', '#0000FF', '#00FFFF', '#00FF00', '#ADFF2F', '#FFFF00', '#FFA500', '#FF4500', '#FF0000']}
        self.ndvi_vis_params = {'min': 0, 'max': 0.7, 'palette': ['#ff0000', '#ffff00', '#00ff00', '#004d00']}
        self.aq_vis_params = {'min': 0, 'max': 100, 'palette': ['#2DC937', '#E7B416', '#E77D11', '#CC3232', '#6B1A6B']}
    ...
    def _calculate_base_layers(self):
        ...
        # 1. Temperature Layer
        self.base_temp = (ee.ImageCollection('MODIS/061/MOD11A1')
                          .filterBounds(self.region).filterDate(*date_range_monthly)
                          .median().select('LST_Day_1km').multiply(0.02).subtract(273.15))
        ...
        aq_components = [
            self._get_normalized_gas('COPERNICUS/S5P/NRTI/L3_NO2', 'NO2_column_number_density', 0.0, 2e-4, date_range_annual),
            self._get_normalized_gas('COPERNICUS/S5P/NRTI/L3_SO2', 'SO2_column_number_density', 0.0, 1e-4, date_range_annual),
            self._get_normalized_gas('COPERNICUS/S5P/NRTI/L3_O3', 'O3_column_number_density', 0.0, 3e-4, date_range_annual),
            self._get_normalized_gas('COPERNICUS/S5P/NRTI/L3_CO', 'CO_column_number_density', 0.0, 3e-2, date_range_annual),
            self._get_normalized_gas('COPERNICUS/S5P/NRTI/L3_AER_AI', 'absorbing_aerosol_index', -1.0, 2.0, date_range_annual)
        ]
        self.base_aq = ee.ImageCollection(aq_components).mean().rename('AQ_Composite_0_100')
    ...
    def _get_normalized_gas(self, coll_id, band, vmin, vmax, date_range):
        ...
        def apply_qa(img):
            ...
            return ee.Image(ee.Algorithms.If(has_qa, main_band_img.updateMask(qa_mask), main_band_img))
        ...
        normalized = collection.subtract(vmin).divide(ee.Number(vmax).subtract(vmin)).multiply(100).clamp(0, 100)
    ...
    def _create_simulated_images(self, preset, ee_geometry):
        ...
        presets = {
            'green_area':  {'temp_op': 'subtract', 'temp_val': 5, 'ndvi_val': 0.7, 'aq_op': 'subtract', 'aq_val': 10},
            'industrial':  {'temp_op': 'add', 'temp_val': 8, 'ndvi_val': 0.05, 'aq_op': 'add', 'aq_val': 40},
            'residential': {'temp_op': 'add', 'temp_val': 4, 'ndvi_val': 0.15, 'aq_op': 'add', 'aq_val': 20}
        }
        ...
        sim_temp_op = self.base_temp.add(config['temp_val']) if config['temp_op'] == 'add' else self.base_temp.subtract(config['temp_val'])
        sim_temp = self.base_temp.where(mask, sim_temp_op)

        sim_ndvi = self.base_ndvi.where(mask, ee.Image.constant(config['ndvi_val']))

        sim_aq_op = self.base_aq.add(config['aq_val']) if config['aq_op'] == 'add' else self.base_aq.subtract(config['aq_val'])
    ...
    def calculate_impact_stats(self, preset, ee_geometry):
        ...
        delta_stats = {key: (post_stats[key] - baseline_stats[key]) if post_stats.get(key) is not None and baseline_stats.get(key) is not None else None
                       for key in baseline_stats}

        return {'preset': preset, 'baseline': baseline_stats, 'post_simulation': post_stats, 'delta': delta_stats}
