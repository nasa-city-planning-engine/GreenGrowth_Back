...
class GeoProcessor:
    ...
    def __init__(
        ...
        print("🌍 Base layers (Temp, NDVI, AQ, Water Quality) calculated and ready.")
    ...
    def _calculate_base_layers(self):
        ...
        b2 = s2_water.select("B2").multiply(0.0001)  # Blue
        b3 = s2_water.select("B3").multiply(0.0001)  # Green
        b4 = s2_water.select("B4").multiply(0.0001)
        b8 = s2_water.select("B8").multiply(0.0001)  # NIR
